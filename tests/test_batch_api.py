...
def test_items_ready_tab_enriches(client, monkeypatch):
    monkeypatch.setattr(bx.items_repo, "page",
                        lambda db, *, statuses, offset, limit, sort_by_product=False, categoryids=None, productid=None: ([_row()], 1))
...
def test_items_in_progress_tab_queries_two_statuses(client, monkeypatch):
    seen = {}
    def fake_page(db, *, statuses, offset, limit, sort_by_product=False, categoryids=None, productid=None):
        seen["statuses"] = statuses
        seen["categoryids"] = categoryids
