...
def test_insert_computes_displayorder_from_count_and_sets_productcolor():
    sink = {}
    row = productimages_repo.insert(
        _InsertDb(sink, count=2), productid="BC1", imageurl="https://public/x.png", productcolor="Red"
    )
    assert sink["table"] == "productimages"
    p = sink["payload"]
    assert p == {"productid": "BC1", "imageurl": "https://public/x.png",
                 "displayorder": 2, "productcolor": "Red", "phototheme": "Default"}
    assert row["imageid"] == 1


def test_insert_omits_productcolor_when_none_and_honors_explicit_displayorder():
    sink = {}
...
def test_insert_carries_explicit_theme():
    ...
    productimages_repo.insert(
        ...
        productcolor="Red", theme="Studio·9:16",
    )
    assert sink["payload"]["phototheme"] == "Studio·9:16"
    assert sink["payload"]["productcolor"] == "Red"
...
def test_delete_for_uses_is_null_when_productcolor_none():
    sink = {}
